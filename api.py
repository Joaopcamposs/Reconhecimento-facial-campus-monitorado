...
def deletar_camera(background_tasks: BackgroundTasks, camera_id: int, session: Session = Depends(get_db)):
    try:
...
def deletar_pessoa(background_tasks: BackgroundTasks, person_id: int, session: Session = Depends(get_db)):
    try:
...
def reconhecimento_facial(camera_id: int, session: Session = Depends(get_db)):
    return StreamingResponse(stream_facial_recognition(session=session, id_camera=camera_id),
